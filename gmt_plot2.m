...
hp1 = fill3(vertxsqO,y2O,ZO,light_gray); %plot outer envelope
hp2 = fill3(vertxsqI,y2I,ZI,dark_gray); %plot inner envolope
...
subplot(2,1,2);
hold on; %xlim = ([1850,2020]);
hp3 = fill3(vertxsqO_diff,y2O_diff,ZO_diff,light_gray);
hp4 = fill3(vertxsqI_diff,y2I_diff,ZI_diff,dark_gray);
...
subplot(2,1,1);
hold on;
hp5 = fill3(vertxsqO,y2O,ZO,light_gray);
hp6 = fill3(vertxsqI,y2I,ZI,dark_gray);
...
subplot(2,1,2)
hold on;
hp7 = fill3(vertxsqO_rec,y2O_rec,ZO_rec,light_gray);
hp8 = fill3(vertxsqI_rec,y2I_rec,ZI_rec,dark_gray);
...
datetick('x','yyyy')
%[hleg,objh] = legend([p9, hp8, hp7, p10, p11],{'GraphEM ensemble median (0.8% sparsity)', 'GraphEM ensemble 25-75%', 'GraphEM ensemble 2.5-97.5%', 'GISTEMP', 'NOAA'});
