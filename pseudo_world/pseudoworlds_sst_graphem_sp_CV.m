%pseudoworlds_sst_graphem_sp_CV.m
...
%% load CV scores
PW1_CV = load('pseudoworld1_sst_CVscores10.mat');
PW2_CV = load('pseudoworld2_sst_CVscores10.mat');
PW3_CV = load('pseudoworld3_sst_CVscores10.mat');
PW4_CV = load('pseudoworld4_sst_CVscores10.mat');
...
%load truth fields
load('pseudoworld1_sst_truth.mat'); PW1_truth = PW1_sst_truth.grid_2d; [ntime, nloc] = size(PW1_truth);
load('pseudoworld2_sst_truth.mat'); PW2_truth = PW2_sst_truth.grid_2d;
load('pseudoworld3_sst_truth.mat'); PW3_truth = PW3_sst_truth.grid_2d;
load('pseudoworld4_sst_truth.mat'); PW4_truth = PW4_sst_truth.grid_2d; [ntime4, nloc4] = size(PW4_truth);


for i = 1:Ncases1 %PW1
loadtag = ['pseudoworld1_sst_graphem_sp' num2str(Scases1(i)*100) '_step2.mat'];
PW1inf{i} = load(loadtag);
PW1inf{i}.Xc = center(PW1inf{i}.Xf);
PW1inf{i}.grid_2d = nan(ntime,nloc); index = PW1inf{i}.PW1_sst.ocean;
...
for i = 1:Ncases2 %PW2
loadtag = ['pseudoworld2_sst_graphem_sp' num2str(Scases2(i)*100) '_step2.mat'];
PW2inf{i} = load(loadtag);
PW2inf{i}.Xc = center(PW2inf{i}.Xf);
PW2inf{i}.grid_2d = nan(ntime,nloc); index = PW2inf{i}.PW2_sst.ocean;
...
for i = 1:Ncases3 %PW3
loadtag = ['pseudoworld3_sst_graphem_sp' num2str(Scases3(i)*100) '_step2.mat'];
PW3inf{i} = load(loadtag);
PW3inf{i}.Xc = center(PW3inf{i}.Xf);
PW3inf{i}.grid_2d = nan(ntime,nloc); index = PW3inf{i}.PW3_sst.ocean;
...
for i = 1:Ncases4 %PW4
loadtag = ['pseudoworld4_sst_graphem_sp' num2str(Scases4(i)*100) '_step2.mat'];
PW4inf{i} = load(loadtag);
PW4inf{i}.Xc = center(PW4inf{i}.Xf);
PW4inf{i}.grid_2d = nan(ntime4,nloc4); index = PW4inf{i}.PW4_sst.ocean;
...
%indavl1 = ~isnan(PW1inf{i}.grid_2d); index1 = PW1inf{i}.PW1_sst.ocean;
%indavl2 = ~isnan(PW2inf{i}.grid_2d); index2 = PW2inf{i}.PW2_sst.ocean;
%indavl3 = ~isnan(PW3inf{i}.grid_2d); index3 = PW3inf{i}.PW3_sst.ocean;
%indavl4 = ~isnan(PW4inf{i}.grid_2d); index4 = PW4inf{i}.PW4_sst.ocean;
...
%% plotting
fig('pseudoworlds sst graphem sp CV scores'); clf;
hold on;
...
%fancyplot_deco('10-fold CV scores for choosing target sparsity (Pseudo-worlds sst)', 'Target sparsity', 'Average MSE', 14, 'Helvetica');
fancyplot_deco('10-fold CV scores for choosing target sparsity (Pseudo-worlds SST)', 'Target sparsity (%)', ['Average MSE (' char(176) 'C^{2})'], 14, 'Helvetica');
...
%hepta_figprint('./figs/pseudoworlds_sst_graphem_sp_CVscores5.eps')
%hepta_figprint('./figs/pseudoworlds_sst_graphem_sp_CVscores.eps')
%hepta_figprint('./figs/pseudoworlds_sst_graphem_sp_CVscores4.eps')
%pause(5)
...
%hepta_figprint('./figs/pseudoworlds_sst_graphem_sp_CVscores3.eps')

%% SAVE AS .TIFF THEN CONVERT TO PDF/JPEG/etc THIS WILL ELIMINATE ANNOYING POLYGONS
filen = './figs/pseudoworlds_sst_graphem_sp_CVscores02'; 
