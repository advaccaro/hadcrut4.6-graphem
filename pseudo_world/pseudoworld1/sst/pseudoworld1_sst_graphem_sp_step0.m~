...
target_vals = [3:.25:4.5]; %target sparsities (start w/ high value to obtain upper limit)
nvals = length(target_vals);


%% Calcualte sample covariance matrix from previosuly infilled dataset
%infilled data
load('pseudoworld1_sst_graphem_cr800_step2.mat')
...
for k = 1:nvals
	...
	adjtag = [odir 'pseudoworld1_sst_adj_sp' num2str(target_spars*100) '.mat'];
