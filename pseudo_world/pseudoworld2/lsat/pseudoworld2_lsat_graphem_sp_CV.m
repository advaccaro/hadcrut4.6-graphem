% pseudoworld2_lsat_graphem_sp_CM.m
...
% Load raw data
raw = load('pseudoworld2_lsat.mat');
Xraw = raw.PW2_lsat.grid_2d; %raw 2D data matrix (w/ missing vals)
Xraw = center(Xraw); [ntime,nspace] = size(Xraw);
loc = raw.PW2_lsat.loc; lats = loc(:,2);
lats_2d = repmat(lats, [1 ntime]);
lats_2d = lats_2d'; %time x space


%Load infilled example to obtain useful meta data
infilled = load('pseudoworld2_lsat_graphem_sp25_step2.mat');
index = infilled.PW2_lsat.land; %index of non-empty grid points
...
for i = 1:Kcv
% Load infilled data
loadtag = ['pseudoworld2_lsat_graphem_sp' num2str(Scases(r)*100) '_step2.mat'];
h4inf{r} = load(loadtag);
%display(['h4inf{' num2str(r) '} = ' loadtag])
h4inf{r}.Xc = center(h4inf{r}.Xf);
land = h4inf{r}.PW2_lsat.land;
...
fig('pseudoworld2 lsat CV scores')
hold on
%plot(Scases,epe,'ko-')
...
savedir = '/home/geovault-02/avaccaro/hadcrut4.6-graphem/pseudo_world/pseudoworld2/lsat/data/';
savetag = 'pseudoworld2_lsat_CVscores10.mat';
