% pseudoworld3_lsat_graphem_sp_CM.m
...
% Load raw data
raw = load('pseudoworld3_lsat.mat');
Xraw = raw.PW3_lsat.grid_2d; %raw 2D data matrix (w/ missing vals)
Xraw = center(Xraw); [ntime,nspace] = size(Xraw);
loc = raw.PW3_lsat.loc; lats = loc(:,2);
lats_2d = repmat(lats, [1 ntime]);
lats_2d = lats_2d'; %time x space


%Load infilled example to obtain useful meta data
infilled = load('pseudoworld3_lsat_graphem_sp25_step2.mat');
index = infilled.PW3_lsat.land; %index of non-empty grid points
nstations = length(index);
indCol = [1:nstations];
[train,test,nin] = kcv_indices(indCol,10);
...
for i = 1:Kcv
% Load infilled data
loadtag = ['pseudoworld3_lsat_graphem_sp' num2str(Scases(r)*100) '_step2.mat'];
h4inf{r} = load(loadtag);
%display(['h4inf{' num2str(r) '} = ' loadtag])
h4inf{r}.Xc = center(h4inf{r}.Xf);
land = h4inf{r}.PW3_lsat.land;
Xin = Xraw(:,land); %select non-empty grid cells
[nt,np] = size(Xin);
indavl1 = ~isnan(Xin);
...
lats_m = lats_2d(:,land); lats_m = lats_m(:,idm);
%lats_in = lats_2d(idm,;);
X_m = Xin(:,idm);
...
fig('pseudoworld3 lsat CV scores')
hold on
%plot(Scases,epe,'ko-')
...
savedir = '/home/geovault-02/avaccaro/hadcrut4.6-graphem/pseudo_world/pseudoworld3/lsat/data/';
savetag = 'pseudoworld3_lsat_CVscores10.mat';
