%pseudoworld3lsat_graphem_sp_step0.m
...
load('pseudoworld3_lsat.mat')
lsat_f = PW3_lsat.grid_2d;
land = ~isnan(lsat_f(1,:));
lsat_l = lsat_f(:,land);


tser = double(PW3_lsat.time) + datenum('1850-1-1');
tvec = datevec(tser);
...
for k = 1:nvals
	...
	adjtag = [odir 'PW3_lsat_adj_sp' num2str(target_spars*100) '.mat'];
