function [explored] = had46ens_explore(infile);
%loads infilled member of HadCRUT4.6 ensemble and prepares it for comparison (calculate GMMT, NINO3.4, etc.)
% only works with infilled members of HadCRUT4.6 (needs correctly formatted input files)

E = load(infile);


% fix grids
lon = double(E.H46ens.lon); lon(lon<0) = lon(lon<0) + 360;
lat = double(E.H46ens.lat);loc = E.loc;
%t=double(E.H46ens.tfrac); %fractional time axis
t = E.H46ens.tser;

lon_in = lon; lat_in = lat; time_in = t;
grid_2d = E.SP1.X;
