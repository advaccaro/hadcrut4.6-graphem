...
datetick('x','yyyy')
[hleg,objh] = legend('HadCRUT4.6 median raw', 'HadCRUT4.6 + GraphEM (0.8% sparsity)');
set(hleg,'Location','SouthEast')
set(objh,'linewidth',2)
legend('boxoff')
fancyplot_deco('Same, difference from HadCRUT 4.6 Cowtan & Way', 'Time (Year)', '{\Delta}T ({\circ}C)',14,'Helvetica')

odir = '/home/geovault-02/avaccaro/hadCRUT4.6/figs/';
ninofig1 = 'had46med_nino_plot1.pdf';
nino1path = [odir ninofig1];
%hepta_figprint(nino1path)
print(nino1path, '-dpdf', '-cmyk', '-bestfit')
...
plot(HG.tmdn,HG.NINO34m,'color',skyblue);
datetick('x','yyyy')
[hleg,objh] = legend('Kaplan','Bunge & Clarke','ERSSTv5','COBE','HadCRUT4.6 + GraphEM (0.8% sparsity)');
set(hleg,'Location','SouthEast')
set(objh,'linewidth',2)
pause;
legend('boxoff')
...
datetick('x','yyyy');
[hleg,objh] = legend('Bunge & Clarke', 'ERSSTv5', 'COBE', 'HadCRUT4.6 + GraphEM (0.8% sparsity)');
set(hleg,'Location','SouthEast')
set(objh,'linewidth',2)
legend('boxoff')
fancyplot_deco('Same, difference to Kaplan', 'Time (Year)', '{\Delta}T ({\circ}C)',14,'Helvetica')

ninofig2 = 'had46med_nino_plot2.pdf';
nino2path = [odir ninofig2];
%hepta_figprint(nino2path)
%print(nino2path, '-dpdf', '-cmyk', '-bestfit')
%print(nino2path, '-dpdf', '-cmyk')
print(nino2path, '-dpdf', '-cmyk', '-fillpage')
